        ...
        if bot_login:
            ...
            if verify_resp.status_code == 200:
                ...
                if bot_login not in assignees:
                    logger.warning(
                        f"Lost race on #{item.issue_number} — assignees are {assignees}, expected {bot_login}"
                    )
                    return False
            else:
                logger.warning(f"Could not verify assignment for #{item.issue_number}: {verify_resp.status_code}")
                return False
